export class ImagePreview {
	...
	setEventListeners = () => {
		...
		const elements = [
			...
			{className: '.brightness-slider', method: event => this.setBrightness(parseInt(event, 10))},
			{className: '.contrast-slider', method: event => this.setContrast(parseInt(event, 10))},
		]
	...
	setContrast = contrast => {
		...
		for (let i = 0; i <imageData.data.length; i+= 4) {
		 imageData.data[i] = editColor(factor * (imageData.data[i] - 128.0) + 128.0);
		 imageData.data[i+1] = editColor(factor * (imageData.data[i + 1] - 128.0) + 128.0);
		 imageData.data[i+2] = editColor(factor * (imageData.data[i + 2] - 128.0) + 128.0);
		}
		this.context.putImageData(imageData, 0, 0);
	  }
